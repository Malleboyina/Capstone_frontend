...
export class CardComponent implements OnInit, OnDestroy {
  ...
  getProds(page: number = 1, size: number = 6) {
    if (this.route.snapshot.url.length == 1) {
      this.productService.getAllInPage(+page, +size)
        .subscribe(page => {
          this.page = page;
          this.title = 'Stay Home Stay Safe';
        });
    } else { //  /category/:id
  ...
  handleAddToWishList( productId:string ,email:string){
    console.log("started");
    
    this.cartService.addToWishList(productId,email).subscribe(() => {
  ...
  handleRemoveFromWishList(productId:string,email:string){
    this.wishListService.removeFromWishList(productId,email).subscribe(() => {
  ...
  sort(){
      ...
      this.page.content.sort((a:any,b:any)=>{
